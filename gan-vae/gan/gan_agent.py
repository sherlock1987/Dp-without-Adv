...
class GanAgent_SAT_WoZ(GanAgent_SAT):
    def __init__(self, corpus, config, action2name):
        super(GanAgent_SAT_WoZ, self).__init__(corpus, config, action2name)
        self.discriminator =gan_model_sat.WoZDiscriminator(config)
        self.generator = gan_model_sat.WoZGenerator(config)
        self.state_out_size = config.state_out_size
        self.noise_factor = 0.3

    def read_real_data(self, sample_shape, batch_feed):
        action_data_feed = one_hot_embedding(batch_feed['action_id'], self.config.action_num)
        real_state_rep = self.np2var(batch_feed['state_onehot'], FLOAT).view(-1, self.state_out_size)
        # real_state_rep = one_hot_embedding(real_state_rep.view(-1), self.config.bucket_num).view(-1, self.state_out_size)
        return real_state_rep, action_data_feed
...
#         state_rep, action_rep = self.read_vae_data(batch_feed)
#         state_action = torch.cat([self.cast_gpu(state_rep), self.cast_gpu(action_rep)], -1)
#         recon_batch, mu, logvar = self.vae(state_action)
#         loss = self.vae_loss(recon_batch, state_action, mu, logvar)
#         vae_loss = Pack(vae_loss=loss) 
#         return vae_loss
        
        
#     def read_vae_data(self,batch_feed):
#         # action_data_feed = one_hot_embedding(batch_feed['action_id'], self.config.action_num)
...
#             BCE = F.binary_cross_entropy(recon_x, x.view(-1, self.vae_in_size), reduction='sum')
